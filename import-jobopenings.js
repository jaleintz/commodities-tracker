...
const seriesId = 'JTSJOL'; // Job Openings: Total Nonfarm
...
async function importJobOpeningsData() {
  try {
    console.log('Fetching FRED Job Openings data...');
    ...
    if (data.observations && data.observations.length > 0) {
      // Save to JSON file
      fs.writeFileSync('FRED_JobOpenings.JSON', JSON.stringify(data, null, 2));
      console.log(`✓ Saved data to FRED_JobOpenings.JSON`);

      // Delete all existing JTSJOL records from the table
      console.log('Clearing existing JTSJOL data from fred_job_openings_tb...');
      ...
      console.log('✓ Successfully cleared existing JTSJOL data');
      ...
      console.log(`Latest job openings: ${parseFloat(data.observations[0].value).toLocaleString()} thousand (${data.observations[0].date})`);
    } else {
