...
const seriesId = 'FEDFUNDS'; // Federal Funds Effective Rate
...
async function importFedFundsData() {
  try {
    console.log('Fetching FRED Federal Funds Rate data...');
    ...
    if (data.observations && data.observations.length > 0) {
      // Save to JSON file
      fs.writeFileSync('FRED_FedFunds.JSON', JSON.stringify(data, null, 2));
      console.log(`✓ Saved data to FRED_FedFunds.JSON`);

      // Delete all existing FEDFUNDS records from the table
      console.log('Clearing existing FEDFUNDS data from fred_fed_funds_rate_tb...');
      ...
      console.log('✓ Successfully cleared existing FEDFUNDS data');
      ...
      console.log(`Latest Federal Funds Rate: ${parseFloat(data.observations[0].value).toLocaleString()}% (${data.observations[0].date})`);
    } else {
