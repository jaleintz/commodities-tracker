...
const seriesId = 'FPCPITOTLZGUSA'; // Inflation, consumer prices for the United States
const url = `https://api.stlouisfed.org/fred/series/observations?series_id=${seriesId}&api_key=${fredApiKey}&file_type=json&sort_order=desc&limit=30`;

async function importInflationData() {
  try {
    console.log('Fetching FRED Inflation data...');
    ...
    if (data.observations && data.observations.length > 0) {
      // Save to JSON file
      fs.writeFileSync('FRED_Inflation.JSON', JSON.stringify(data, null, 2));
      console.log(`✓ Saved data to FRED_Inflation.JSON`);

      // Delete all existing FPCPITOTLZGUSA records from the table
      console.log('Clearing existing FPCPITOTLZGUSA data from fred_inflation_tb...');
      ...
      console.log('✓ Successfully cleared existing FPCPITOTLZGUSA data');
      ...
      console.log(`Latest Inflation Rate: ${parseFloat(data.observations[0].value).toLocaleString()}% (${data.observations[0].date})`);
    } else {
