...
const seriesId = 'M2SL'; // M2 Money Stock
...
async function importM2Data() {
  try {
    console.log('Fetching FRED M2 Money Stock data...');
    ...
    if (data.observations && data.observations.length > 0) {
      // Save to JSON file
      fs.writeFileSync('FRED_M2.JSON', JSON.stringify(data, null, 2));
      console.log(`✓ Saved data to FRED_M2.JSON`);

      // Delete all existing M2SL records from the table
      console.log('Clearing existing M2SL data from fred_m2_money_stock_tb...');
      ...
      console.log('✓ Successfully cleared existing M2SL data');
      ...
      console.log(`Latest M2 Money Stock: $${parseFloat(data.observations[0].value).toLocaleString()} billion (${data.observations[0].date})`);
    } else {
