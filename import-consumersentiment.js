...
async function importConsumerSentimentData() {
  try {
    console.log('Fetching FRED Consumer Sentiment data...');
    ...
    if (data.observations && data.observations.length > 0) {
      // Save to JSON file
      fs.writeFileSync('FRED_ConsumerSentiment.JSON', JSON.stringify(data, null, 2));
      console.log(`✓ Saved data to FRED_ConsumerSentiment.JSON`);

      // Delete all existing UMCSENT records from the table
      console.log('Clearing existing UMCSENT data from fred_consumer_sentiment_tb...');
      ...
      console.log('✓ Successfully cleared existing UMCSENT data');
      ...
      console.log(`Latest Consumer Sentiment: ${parseFloat(data.observations[0].value).toLocaleString()} (${data.observations[0].date})`);
    } else {
