...
const seriesId = 'VIXCLS'; // CBOE Volatility Index: VIX
...
async function importVixData() {
  try {
    console.log('Fetching FRED VIX data...');
    ...
    if (data.observations && data.observations.length > 0) {
      // Save to JSON file
      fs.writeFileSync('FRED_VIX.JSON', JSON.stringify(data, null, 2));
      console.log(`✓ Saved data to FRED_VIX.JSON`);

      // Delete all existing VIXCLS records from the table
      console.log('Clearing existing VIXCLS data from fred_vix_tb...');
      ...
      console.log('✓ Successfully cleared existing VIXCLS data');
      ...
      console.log(`Latest VIX: ${parseFloat(data.observations[0].value).toLocaleString()} (${data.observations[0].date})`);
    } else {
